...
- **Компонентная база:** shadcn/ui (Radix UI primitives: `@radix-ui/react-dialog`, `@radix-ui/react-slot`, `@radix-ui/react-tooltip`) + `cn` (официальный ультрабыстрый движок от shadcn/ui)
- **Иконки:** Hugeicons (`@hugeicons/react`, `@hugeicons/core-free-icons`)
- **QR-генерация:** `qr-code-styling` (стилизованный рендеринг)
...
## 4. Потенциал и планы развития (Roadmap)

1. **Генерация бланков и заявлений в PDF на клиенте ([pdfcn](https://github.com/shadcn-labs/pdfcn)):**
   - Возможность для студента ввести свои данные (ФИО, курс, группа) и скачать готовое сформированное заявление/бланк в формате PDF прямо в браузере без обращения к бэкенду. Библиотека `pdfcn` создана специально для shadcn/ui (на базе движков Takumi / Forme) и использует тот же CLI и систему компонентов.
2. **Быстрое копирование реквизитов (Sonner Toasts):**
   - Копирование номера кабинета или ссылки в буфер обмена в 1 клик с показом аккуратного тост-уведомления в стиле Apple.
3. **Клиентский поиск и фильтрация:**
   - При дальнейшем расширении каталога справок — подключение строки быстрого поиска и фильтрации по ключевым словам и кабинетам.
